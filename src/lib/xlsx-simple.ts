...
/** Minimal XLSX (Excel) from a header row + data rows. */
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="${xmlText(safeSheet)}" sheetId="1" r:id="rId1"/></sheets>
</workbook>`
  );
    ...
    .map((row, rIdx) => {
      ...
      const cells = row
        .map((val, cIdx) => {
          ...
          return `<c r="${ref}" t="inlineStr"><is><t xml:space="preserve">${xmlText(val ?? "")}</t></is></c>`;
        })
        .join("");
